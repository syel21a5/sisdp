...
class AdministrativoController extends Controller
{
    ...
    private function exportarExcel($tipo, $filtros, $nomeArquivo)
    {
        switch ($tipo) {
            case 'geral':
                return Excel::download(new AdministrativoGeralExport($filtros), $nomeArquivo . '.xlsx');
            case 'crimes':
                return Excel::download(new AdministrativoCrimesExport($filtros), $nomeArquivo . '.xlsx');
            case 'pessoas':
                return Excel::download(new AdministrativoPessoasExport($filtros), $nomeArquivo . '.xlsx');
            case 'apreensoes':
                return Excel::download(new AdministrativoApreensoesExport($filtros), $nomeArquivo . '.xlsx');
            default:
                return response()->json(['success' => false, 'message' => 'Tipo de relatório não suportado'], 400);
        }
    }
    ...
    private function exportarPdf($tipo, $filtros, $nomeArquivo)
    {
        ...
        $dompdf->render();
        return $dompdf->stream($nomeArquivo . '.pdf', ['Attachment' => false]);
    }
