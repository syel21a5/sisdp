...
class GerarPdf_RoldeTestemunhas_Controller extends Controller
{
    /**
     * ✅ MÉTODO PRINCIPAL PARA GERAR PDF
     */
    public function gerarPdfRoldeTestemunhas(Request $request)
    {
        return $this->processarPdf($request, 'rol-de-testemunhas-');
    }
    ...
    private function processarPdf(Request $request, $prefixoArquivo)
    {
        ...
        // ✅ Se os dados chegarem apenas na variável 'dados' (como string JSON)
        if (isset($dados['dados']) && is_string($dados['dados'])) {
            $dadosArray = json_decode($dados['dados'], true);
            if (is_array($dadosArray)) {
                $dados = array_merge($dados, $dadosArray);

                // Se o conteúdo não veio no request principal mas está no JSON
                if (!isset($dados['conteudo']) && isset($dadosArray['conteudo'])) {
                    $dados['conteudo'] = $dadosArray['conteudo'];
                }
            }
        }

        // Se ainda não tiver conteúdo, tenta pegar do input 'conteudo'
        if (empty($dados['conteudo'])) {
            $dados['conteudo'] = $request->input('conteudo');
        }

        // Decodificar todos os dados usando método da classe
        $dadosCondutor = $this->decodificarDados($dados, 'condutor');
        $dadosVitima1 = $this->decodificarDados($dados, 'vitima1');
        $dadosVitima2 = $this->decodificarDados($dados, 'vitima2');
        $dadosVitima3 = $this->decodificarDados($dados, 'vitima3');
        $dadosTestemunha1 = $this->decodificarDados($dados, 'testemunha1');
        $dadosTestemunha2 = $this->decodificarDados($dados, 'testemunha2');
        $dadosTestemunha3 = $this->decodificarDados($dados, 'testemunha3');
        $dadosAutor1 = $this->decodificarDados($dados, 'autor1');
        $dadosAutor2 = $this->decodificarDados($dados, 'autor2');
        $dadosAutor3 = $this->decodificarDados($dados, 'autor3');
        ...
        <head>
            ...
            <style>
                ...
                .orgao-principal { font-weight: bold; font-size: 12.5pt; margin-bottom: 2px; line-height: 1.1; }
                .orgao-secundario { font-size: 10pt; margin-bottom: 1px; line-height: 1.1; }
                .delegacia-info { margin-top: 3px; font-weight: bold; font-size: 11.5pt; line-height: 1.1; }
                .header table { margin-bottom: 0 !important; width: 100%; border-collapse: collapse; }
                .header td { padding: 0 5px !important; vertical-align: middle; }
                .header img { width: 85px !important; height: 85px !important; margin-top: -3px !important; object-fit: contain; }
                .conteudo-pagina { margin-top: 15px !important; padding: 0 !important; }
                .ql-font-arial, .ql-font-arial p, .ql-font-arial span, .ql-font-arial div, .ql-font-arial strong { font-family: Arial, sans-serif !important; }
                .ql-font-times-new-roman, .ql-font-times-new-roman p, .ql-font-times-new-roman span, .ql-font-times-new-roman div, .ql-font-times-new-roman strong { font-family: "Times New Roman", Times, serif !important; }
        ...
        <body>
            <div class="header">
                <table>
                    <tr>
                        <td style="width: 85px; text-align: right; vertical-align: middle; padding-right: 15px;">
                        ...
                        <td style="width: 85px; text-align: left; vertical-align: middle; padding-left: 15px;">
        ...
        return $dompdf->stream($prefixoArquivo . date('Y-m-d-His') . '.pdf', [
            'Attachment' => false
        ]);
    }
     ...
     */
    private function decodificarDados($dados, $chave)
    {
    ...
    private function cleanContent($content)
    {
        ...
        // Remover classes do Quill que podem vazar para o PDF
        $content = preg_replace('/<div class="ql-editor"([^>]*)>/', '', $content);
        $content = preg_replace('/<div class="ql-clipboard"([^>]*)>.*?<\/div>/s', '', $content);
        $content = preg_replace('/<div class="ql-tooltip"([^>]*)>.*?<\/div>/s', '', $content);

        // Se removeu o div ql-editor, precisa remover o fechamento dele
        // Mas apenas se for o último div do conteúdo (para não quebrar outros divs legítimos)
