...
class GerarPdf_AvaliacaoIndireta_Controller extends Controller
{
    // ✅ MÉTODO PARA PORTARIA
    public function gerarPdfAvaliacaoPortaria(Request $request)
    {
        return $this->processarPdfAvaliacao($request, 'portaria');
    }

    // ✅ MÉTODO PARA TERMO
    public function gerarPdfAvaliacaoTermo(Request $request)
    {
        return $this->processarPdfAvaliacao($request, 'termo');
    }

    // ✅ MÉTODO PRIVADO UNIFICADO (nome alterado para evitar conflito)
    private function processarPdfAvaliacao(Request $request, $tipo)
    {
...
<body>
    ...
    <div class="content">' . $dados['conteudo'] . '</div>
        ...
        $nomeArquivo = $tipoDocumento === 'portaria'
            ? 'portaria-avaliacao-indireta-' . date('Y-m-d-His') . '.pdf'
            : 'termo-avaliacao-indireta-' . date('Y-m-d-His') . '.pdf';
