...
// ✅✅✅ FUNÇÃO PRINCIPAL CORRIGIDA - DETECTA TIPO DE DOCUMENTO ✅✅✅
function printDocument() {
    console.log('Iniciando geração de PDF - Exame de Constatação de Danos...');
    console.log('Dados disponíveis:', window.dadosParaImpressao);

    const dados = window.dadosParaImpressao || {};

    // ✅ DETECTAR SE É PORTARIA OU TERMO
    // Tenta detectar pelo tipo definido globalmente ou infere da URL
    ...
    // Prioriza a rota definida na view (window.rotaPdf)
    // Se não existir, usa a lógica legado
    let formAction = window.rotaPdf;
    
    if (!formAction) {
         formAction = isPortaria
            ? '/exame-danos-portaria'
            : '/exame-danos-termo';
    }

    console.log('Enviando para rota:', formAction);
    ...
    // ✅✅✅ CORREÇÃO CRÍTICA: Enviar dados individuais em vez de JSON
    // Enviar cada campo individualmente para facilitar o processamento no controller
    const dadosParaEnviar = {
        'delegacia': dados.delegacia || 'NÃO INFORMADO',
        'cidade': dados.cidade || 'NÃO INFORMADO',
        'delegado': dados.delegado || 'NÃO INFORMADO',
        'escrivao': dados.escrivao || 'NÃO INFORMADO',
        ...
        'boe': dados.boe || '', // Adicionado BOE se disponível
        'nome': dados.nome || '' // Adicionado Nome se disponível
    };
    ...
    Object.keys(dadosParaEnviar).forEach(key => {
        ...
        console.log(`Enviando ${key}: ${dadosParaEnviar[key]}`);
    });
...
// ✅ CONFIGURAÇÃO INICIAL MOVIDA PARA initializeEditor
...
function initializeEditor() {
    ...
    console.log('Tipo de documento:', window.location.pathname);
    ...
    const printButton = document.querySelector('button.btn-custom');
    if (printButton) {
        // Remover listeners anteriores (clone do elemento) para evitar duplicidade
        const newButton = printButton.cloneNode(true);
        printButton.parentNode.replaceChild(newButton, printButton);

        newButton.textContent = 'Gerar PDF';
        newButton.addEventListener('click', printDocument);
    }
