...
class PromptGeneratorController extends Controller
{
    ...
    public function extrairDadosComIA(Request $request)
    {
        ...
        try {
            ...
            // Normalizar chaves para o formato esperado pelo frontend (compatibilidade com motor Nativo)
            if (isset($dados['data_fato']) && !isset($dados['data'])) {
                $dados['data'] = $dados['data_fato'];
            }
            if (!isset($dados['ip'])) {
                $dados['ip'] = '';
            }
