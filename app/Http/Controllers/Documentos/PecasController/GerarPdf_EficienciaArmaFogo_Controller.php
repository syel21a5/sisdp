...
class GerarPdf_EficienciaArmaFogo_Controller extends Controller
{
    // ✅ MÉTODO PARA PORTARIA
    public function gerarPdfEficienciaArmaPortaria(Request $request)
    {
        return $this->processarPdfEficienciaArma($request, 'portaria');
    }

    // ✅ MÉTODO PARA TERMO
    public function gerarPdfEficienciaArmaTermo(Request $request)
    {
        return $this->processarPdfEficienciaArma($request, 'termo');
    }

    // ✅ MÉTODO PRIVADO UNIFICADO (Baseado no Avaliacao para consistência)
    private function processarPdfEficienciaArma(Request $request, $tipo)
    {
...
<head>
    ...
    <style>
        /* MARGEM PADRÃO IDÊNTICA AO TERMO DE INTERROGATÓRIO */
...
<body>
    ...
    <div class="content">' . $dados['conteudo'] . '</div>
        ...
        return PdfService::generatePdf($html, 'eficiencia-arma-fogo-' . date('Y-m-d-His') . '.pdf');
    }
