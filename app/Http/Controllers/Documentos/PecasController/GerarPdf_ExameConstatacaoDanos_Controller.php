...
class GerarPdf_ExameConstatacaoDanos_Controller extends Controller
{
    // ✅ MÉTODO PARA PORTARIA
    public function gerarPdfExameDanosPortaria(Request $request)
    {
        return $this->processarPdfExameDanos($request, 'portaria');
    }

    // ✅ MÉTODO PARA TERMO
    public function gerarPdfExameDanosTermo(Request $request)
    {
        return $this->processarPdfExameDanos($request, 'termo');
    }

    // ✅ MÉTODO PRIVADO UNIFICADO (nome alterado para evitar conflito)
    private function processarPdfExameDanos(Request $request, $tipo)
    {
        ...
        Log::info('Dados recebidos para PDF Exame Danos:', $dados);
...
<body>
    ...
    <div class="content">' . ($dados['conteudo'] ?? '') . '</div>
        ...
        return PdfService::generatePdf($html, 'exame-constatacao-danos-' . date('Y-m-d-His') . '.pdf');
    }

    // ✅ MÉTODO ORIGINAL PARA COMPATIBILIDADE (mantido para não quebrar outras partes do sistema)
    public function gerarPdfAvaliacao(Request $request)
    {
        // Por padrão, redireciona para a portaria para manter compatibilidade
        return $this->processarPdfExameDanos($request, 'portaria');
    }
