        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response.success) {
                    ...
                    // ✅ FALLBACK PMPE: Adiciona qualquer pessoa encontrada nos detalhes que não foi mapeada para um papel específico
                    if (dados.envolvidos_detalhes) {
                        if (!OcorrenciasApp.envolvidos.outros) OcorrenciasApp.envolvidos.outros = [];
                        let addedOutros = false;
                        
                        Object.keys(dados.envolvidos_detalhes).forEach(nomeChave => {
                            let jaExiste = false;
                            ['vitimas', 'autores', 'testemunhas', 'condutores', 'outros'].forEach(tipo => {
                                if (OcorrenciasApp.envolvidos[tipo] && OcorrenciasApp.envolvidos[tipo].some(p => OcorrenciasApp.normalizarNome(p) === OcorrenciasApp.normalizarNome(nomeChave))) {
                                    jaExiste = true;
                                }
                            });
                            
                            if (!jaExiste) {
                                OcorrenciasApp.envolvidos.outros.push(nomeChave.trim());
                                addedOutros = true;
                            }
                        });
