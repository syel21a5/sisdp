...
$(document).ready(function () {
    ...
    function inicializarVeiculo() {
        ...
        // ✅ NOVO: Lógica Centralizada usando o CoreExtractor (IA e Nativo)
        if (typeof CoreExtractor !== 'undefined') {
            new CoreExtractor({
                suffix: 'Veiculo',
                extractUrl: rotasVeiculo.importarBoeTexto || '/veiculo/importar-boe-texto',
                extractUrlIA: '/prompt/extrair-ia', // Usa a IA padrão
                onSuccess: function(response, motor) {
                    if (response.success && response.dados) {
                        ...
                        // O motor IA pode retornar 'veiculos_detalhes' ou 'veiculos', unificamos aqui:
                        let veiculosList = dados.veiculos || dados.veiculos_detalhes || [];
                        
                        if (veiculosList && veiculosList.length > 0) {
                            if (veiculosList.length === 1) {
                                // APENAS UM VEÍCULO DETECTADO - Tentar preencher direto
                                if (dados.boe) {
                                    $.ajax({
                                        url: rotasVeiculo.pesquisar,
                                        method: "GET",
                                        ...
                                        success: function(pesqResp) {
                                            ...
                                            let placaLocal = veiculosList[0].placa ? veiculosList[0].placa.replace(/[^a-zA-Z0-9]/g, '').toUpperCase() : '';
                                            let chassiLocal = veiculosList[0].chassi ? veiculosList[0].chassi.replace(/[^a-zA-Z0-9]/g, '').toUpperCase() : '';
                                            let specLocal = ((veiculosList[0].marca_modelo || '') + ' ' + (veiculosList[0].cor || '')).replace(/[^a-zA-Z0-9]/g, '').toUpperCase();
                                            ...
                                            for (let dbItem of itensDB) {
                                                ...
                                                if ((placaLocal && placaLocal !== 'SPLACA' && dbPlaca === placaLocal) || 
                                                    (chassiLocal && chassiLocal !== 'SCHASSI' && dbChassi === chassiLocal)) {
                                                    jaExisteExato = true;
                                                    break;
                                                }
                                                
                                                if ((!placaLocal || placaLocal === 'SPLACA') && (!dbPlaca || dbPlaca === 'SPLACA') && 
                                                    (!chassiLocal || chassiLocal === 'SCHASSI') && (!dbChassi || dbChassi === 'SCHASSI')) {
                                            ...
                                            if (jaExisteExato) {
                                                ...
                                                if (jaExisteSimilar) {
                                                    ...
                                                    window.mostrarSucesso(`Extração concluída via ${motor.toUpperCase()}! 1 veículo detectado e preenchido no formulário.`);
                                                }
                                            }
                                        },
                                        error: function() {
                                            $('#modalImportarVeiculo').modal('hide');
                                            preencherComObjetoIAVeiculo(veiculosList[0]);
                                            window.mostrarSucesso(`Extração concluída via ${motor.toUpperCase()}! 1 veículo detectado.`);
                                        }
                                    });
                                } else {
                                    $('#modalImportarVeiculo').modal('hide');
                                    preencherComObjetoIAVeiculo(veiculosList[0]);
                                    window.mostrarSucesso(`Extração concluída via ${motor.toUpperCase()}! 1 veículo detectado e preenchido.`);
                                }
                            } else {
                    ...
                    // Limpar o textarea após o sucesso da extração
                    $('#textoBoeVeiculo').val('');
                }
            });
        } else {
            console.error('CoreExtractor não está carregado! Verifique a importação no Blade.');
        }
