...
const quill = new Quill('#editor', {
        ...
        toolbar: {
            ...
            handlers: {
                'titlebar': function () {
                    ...
                    if (!range) return;
                    const [block] = this.quill.scroll.descendant(Quill.import('parchment').Block, range.index);
                    if (!block || !block.domNode) return;
                    const el = block.domNode;
                    const applied = el.getAttribute('data-titlebar') === '1';
                    if (applied) {
                        el.removeAttribute('data-titlebar');
                        el.style.backgroundColor = '';
                        el.style.border = '';
                        el.style.padding = '';
                        el.style.fontWeight = '';
                    } else {
                        el.setAttribute('data-titlebar', '1');
                ...
                // Handler para Desfazer
                'undo': function () {
                    this.quill.history.undo();
                },

                // Handler para Refazer
                'redo': function () {
    ...
    formats: [
        ...
        'link', 'image', 'list', 'bullet', 'indent', 'align',
        'color', 'background'
    ]
});
...
function adjustInitialSpacing() {
    ...
    editorElement.querySelectorAll('p').forEach(p => {
        ...
        // p.style.padding = '0'; // REMOVIDO: Impedia o funcionamento do recuo (indent)
...
function cleanHtmlContent(html) {
    ...
    // 8. Removido: não inserir quebra de página automaticamente neste ofício (documento de 1 página)
...
function printDocument() {
    ...
    form.method = 'POST';
    form.action = '/despacho-de-conclusao-'; // ✅ CORRIGIDO: Rota POST correta
    form.target = '_blank';
    ...
    const dadosParaEnviar = {
        ...
        'data_comp': dados.data_comp || 'NÃO INFORMADO', // ✅ ADICIONADO: Campo data_comp
        ...
        // ✅ Dados das pessoas (JSON) - DINÂMICO
    ...
    // ✅ INJETAR TODAS AS TESTEMUNHAS DINAMICAMENTE
    Object.keys(dados).forEach(key => {
        if (key.match(/^testemunha\d+$/)) {
            dadosParaEnviar[key] = JSON.stringify(dados[key] || {});
        }
        // Injetar vítimas dinamicamente também, se necessário
        if (key.match(/^vitima\d+$/) && !dadosParaEnviar[key]) {
            dadosParaEnviar[key] = JSON.stringify(dados[key] || {});
        }
    });
...
// ✅ CORRIGIDO: Console.log com nome correto
console.log('🟢 BLADE OFÍCIOS MP CARREGADO');
console.log('🟢 Rota MP Ofícios carregada com sucesso');
