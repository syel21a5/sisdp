...
function cleanHtmlContent(html) {
    ...
    const isUnico = (window.dadosParaImpressao && window.dadosParaImpressao.tipo === 'apfd_unico') || (typeof window !== 'undefined' && window.location && window.location.pathname && window.location.pathname.indexOf('oficios-apfd-unico') !== -1);
    if (!isUnico && !html.includes('page-break') && html.includes('Delegado(a) de Polícia</p>')) {
...
function printDocument() {
    ...
    form.method = 'POST';
    // ✅ USAR A ROTA DINÂMICA
    form.action = '/oficios-apfd-dinamico';
    form.target = '_blank';
    ...
    const dadosParaEnviar = {
        ...
        // ✅ LISTA DE AUTORES DINÂMICA
        'autores': JSON.stringify(dados.autores || [])
    };
