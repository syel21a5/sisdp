...
<head>
    ...
    <title>APFD AUTOR 3 - Editor Profissional</title>
...
<body class="body-declaracao">
        ...
        <div class="editor-area">
            ...
            <div style="line-height: 1.6; margin: 0.2em 0px; padding: 0px; text-align: left;">
                <p style="line-height: 1.6; margin: 0.2em 0px; padding: 0px; text-align: justify;">
                        Prosseguindo, passou a Autoridade a qualificar e inquirir o <strong>CONDUZIDO</strong>: {{ !empty($dadosArray['nome']) ? $dadosArray['nome'] : 'NÃO INFORMADO' }},
                        ...
                        <strong>O qual depois de cientificado das imputações que lhe são feitas e dos direitos assegurados pelo Art. 5º, caput e
                        incisos LXI, LXII, LXIII, LXIV, LXV e LXVI, da Constituição Federal</strong>, do respeito à sua integridade física e moral, de não ser
                        preso senão em flagrante delito ou por ordem escrita e fundamentada de autoridade judiciária competente, de ter sua prisão e o local onde
                        se encontre comunicados imediatamente, ao juiz competente e à sua família, ou à pessoa por ele indicada, de permanecer calado, sendo-lhe
                        assegurada a assistência da família e de advogado e a identificação dos responsáveis por sua prisão ou por seu interrogatório policial,
                        expressou a vontade de que sua prisão seja comunicada a pessoa de seu (sua)
                        <strong>{{ !empty($dadosArray['parente']) ? $dadosArray['parente'] : (!empty($dadosArray['Parente']) ? $dadosArray['Parente'] : 'NÃO INFORMADO') }}</strong>, a saber:
                        <strong>{{ !empty($dadosArray['familia']) ? $dadosArray['familia'] : (!empty($dadosArray['Familia']) ? $dadosArray['Familia'] : 'NÃO INFORMADO') }}</strong>, residente no endereço supra.
                        <strong>Interrogado na forma do Art. 187, §§ 1º e 2º do Código de Processo Penal, RESPONDEU</strong>: QUE, não tem advogado nem
                        indica um para este ato; QUE, _________________;
                </p>
                ...
                <div class="assinatura-area">
                    ...
                    <p style="line-height: 1.4; margin: 0.1em 0; padding: 0;">CONDUZIDO(A):</p>
                    <p style="line-height: 1.4; margin: 0.1em 0; padding: 0;">&nbsp;</p>
    ...
    <div id="dados-apfd" data-json='<?= json_encode(isset($dadosArray) ? $dadosArray : [], JSON_UNESCAPED_UNICODE) ?>'></div>
    <script>
    (function(){
        var el = document.getElementById('dados-apfd');
        var raw = el ? el.getAttribute('data-json') : '{}';
        try { window.dadosParaImpressao = JSON.parse(raw || '{}'); } catch(e) { window.dadosParaImpressao = {}; }
    })();
    </script>

    <!-- JavaScript principal - CAMINHO ALTERADO -->
    <script src="{{ asset('js/pages/apfd/apfd_autor3.js') }}"></script>
</body>
</html>
