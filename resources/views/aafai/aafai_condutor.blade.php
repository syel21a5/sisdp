...
if (isset($dadosArray['autores'])) {
    ...
    if (is_string($rawAutores)) {
        $autores = json_decode($rawAutores, true) ?? [];
    } elseif (is_array($rawAutores)) {
        $autores = $rawAutores;
    }
}

// Fallback: se não houver chips, usar autor1...autor5
if (empty($autores)) {
    for ($i = 1; $i <= 5; $i++) {
        if (isset($dadosArray["autor$i"]) && is_array($dadosArray["autor$i"])) {
            $nome = $dadosArray["autor$i"]['nome'] ?? '';
            if ($nome && !in_array(strtoupper($nome), ['AUTOR 1', 'AUTOR 2', 'AUTOR 3', 'AUTOR 4', 'AUTOR 5', 'NÃO INFORMADO', ''])) {
...
// Se ainda estiver vazio, usar autor1 como padrão
if (empty($autores)) {
...
if (isset($dadosArray['vitimas'])) {
    ...
    if (is_string($rawVitimas)) {
        $vitimas = json_decode($rawVitimas, true) ?? [];
...
if (empty($vitimas)) {
    for ($i = 1; $i <= 3; $i++) {
        if (isset($dadosArray["vitima$i"]) && is_array($dadosArray["vitima$i"])) {
            $nome = $dadosArray["vitima$i"]['nome'] ?? '';
            if ($nome && !in_array(strtoupper($nome), ['VITIMA 1', 'VITIMA 2', 'VITIMA 3', 'NÃO INFORMADO', ''])) {
...
if (isset($dadosArray['testemunhas'])) {
    ...
    if (is_string($rawTestemunhas)) {
        $testemunhas = json_decode($rawTestemunhas, true) ?? [];
...
if (empty($testemunhas)) {
    for ($i = 1; $i <= 3; $i++) {
        if (isset($dadosArray["testemunha$i"]) && is_array($dadosArray["testemunha$i"])) {
            $nome = $dadosArray["testemunha$i"]['nome'] ?? '';
            if ($nome && !in_array(strtoupper($nome), ['TESTEMUNHA 1', 'TESTEMUNHA 2', 'TESTEMUNHA 3', 'NÃO INFORMADO', ''])) {
...
if (count($testemunhasNomes) === 1) {
    ...
    <div class="editor-wrapper">
        ...
        <script>
            ...
            <script>
                console.log('🔵 BLADE AAFAI CONDUTOR CARREGADO');
                console.log('🔵 Rota:', '{{ request()->path() }}');
                console.log('🔵 Deveria carregar JS:', '{{ asset('js/pages/aafai/aafai_condutor.js') }}');
        </script>
