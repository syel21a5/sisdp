...
<head>
    ...
    <title>Ofício de Perícia Papiloscópica em Pessoa - Editor Profissional</title>
    ...
    <div class="editor-wrapper">
        <div class="editor-header">
            <h1 class="editor-title">
                ...
                Editor: PERÍCIA PAPILOSCÓPICA EM PESSOA
            </h1>
        </div>
        ...
        <div class="editor-area">
            <div id="editor" class="preservar-espacamento">
                ...
                @php
                    ...
                    // Pega as vítimas selecionadas no modal
                    $vitimasArr = !empty($dadosArray['lista_vitimas']) ? $dadosArray['lista_vitimas'] : (!empty($dadosArray['vitimas']) ? $dadosArray['vitimas'] : []);
                    foreach($vitimasArr as $v) {
                        if (!empty($v['nome'])) $pessoasSelecionadas[] = strtoupper($v['nome']);
                    }

                    // Pega os autores selecionados no modal
                    $autoresArr = !empty($dadosArray['lista_autores']) ? $dadosArray['lista_autores'] : (!empty($dadosArray['autores']) ? $dadosArray['autores'] : []);
                    foreach($autoresArr as $a) {
                        if (!empty($a['nome'])) $pessoasSelecionadas[] = strtoupper($a['nome']);
                    }

                    if (count($pessoasSelecionadas) > 0) {
                        $alvo = implode(', ', $pessoasSelecionadas);
                    } elseif (!empty($dadosArray['vitima1_nome'])) {
                        $alvo = strtoupper($dadosArray['vitima1_nome']);
                    } elseif (!empty($dadosArray['autor_nome'])) {
                        $alvo = strtoupper($dadosArray['autor_nome']);
                    }
                @endphp
                <p style="text-align: justify; line-height: 1.5;">
                    Cumprimentando-o cordialmente, solicito a V. Exª a realização de <strong>PERÍCIA PAPILOSCÓPICA</strong> na pessoa de <strong>{{ $alvo }}</strong>, para fins de identificação/qualificação, estando os fatos vinculados ao presente Boletim de Ocorrência.
                </p>
