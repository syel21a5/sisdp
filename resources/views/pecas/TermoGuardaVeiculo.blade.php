...
<head>
    ...
    <title>Termo de Guarda e Entrega de Veículo - Editor Profissional</title>
...
<body class="body-declaracao">
    <div class="editor-wrapper">
        <div class="editor-header">
            <h1 class="editor-title">
                ...
                Editor: TERMO DE GUARDA E ENTREGA DE VEÍCULO
            </h1>
        </div>
        ...
        <div class="editor-area">
            <div id="editor" class="preservar-espacamento">
                ...
                <p style="text-align: center; font-size: 18px; line-height: 1.5; margin: 30px 0;">
                    <strong>TERMO DE GUARDA E ENTREGA DE VEÍCULO</strong>
                </p>
                ...
                <p style="text-align: justify; line-height: 1.8; text-indent: 40px; margin-bottom: 20px;">
                    Aí sendo, compareceu a pessoa de <strong>{{ mb_strtoupper($dadosArray['veiculo_recebedor_nome'] ?? 'NOME DO RECEBEDOR') }}</strong>, portador(a) do RG nº {{ $dadosArray['veiculo_recebedor_rg'] ?? '_____' }} e CPF nº {{ $dadosArray['veiculo_recebedor_cpf'] ?? '_____' }}, residente e domiciliado(a) na(o) {{ $dadosArray['veiculo_recebedor_endereco'] ?? '_____' }}, o(a) qual requereu a restituição do veículo abaixo discriminado, que fora apreendido por ocasião da ocorrência em epígrafe.
                </p>

                <p style="text-align: justify; line-height: 1.8; margin-left: 40px; margin-bottom: 20px;">
                    <strong>Veículo:</strong> {{ mb_strtoupper($dadosArray['veiculo_tipo'] ?? 'AUTOMÓVEL') }}, marca <strong>{{ mb_strtoupper($dadosArray['veiculo_marca'] ?? '') }}</strong>, modelo <strong>{{ mb_strtoupper($dadosArray['veiculo_modelo'] ?? '') }}</strong>, cor <strong>{{ mb_strtoupper($dadosArray['veiculo_cor'] ?? '') }}</strong>, placa <strong>{{ mb_strtoupper($dadosArray['veiculo_placa'] ?? '') }}</strong>.
                </p>

                <p style="text-align: justify; line-height: 1.8; text-indent: 40px; margin-bottom: 50px;">
                    Pela Autoridade Policial foi deferido o pedido, nomeando-o(a) como <strong>FIEL DEPOSITÁRIO(A)</strong> do referido veículo, que lhe foi entregue neste ato, assumindo o compromisso de não vendê-lo, trocá-lo, alugá-lo, ocultá-lo ou dele se desfazer sem prévia autorização judicial, bem como de apresentá-lo perante a Autoridade Policial ou Judiciária sempre que requisitado, sob as penas do crime de desobediência e/ou apropriação indébita. E, por estarem justos e acertados, assinam o presente termo.
                </p>
                    ...
                    <table style="width: 100%; border: none;">
                        <tr>
                            ...
                            <td style="width: 50%; text-align: center; border: none; padding-left: 20px; vertical-align: top;">
                                ________________________________________<br>
                                <strong>{{ mb_strtoupper($dadosArray['veiculo_recebedor_nome'] ?? 'NOME DO RECEBEDOR') }}</strong><br>
                                Recebedor / Fiel Depositário(a)
                            </td>
                        </tr>
                        <tr>
    ...
    <script src="{{ asset('js/pages/pecas/TermoGuardaVeiculo.js') }}?v={{ time() }}"></script>
</body>
</html>
